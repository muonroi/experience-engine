...
const __srcdir = require('node:path').join(__dirname, '../../.experience');
...
const {
  ...
  getHookTimeoutMs,
  getExtractTimeoutMs,
} = require('../../.experience/remote-client');
...
test('exp-client-drain flushes queued extract events', async () => {
  const { homeDir } = withTempHome();
  const expDir = path.join(homeDir, '.experience');
  fs.copyFileSync(path.join(__srcdir, 'exp-client-drain.js'), path.join(expDir, 'exp-client-drain.js'));
  fs.copyFileSync(path.join(__srcdir, 'remote-client.js'), path.join(expDir, 'remote-client.js'));
  fs.copyFileSync(path.join(__srcdir, 'extract-compact.js'), path.join(expDir, 'extract-compact.js'));
