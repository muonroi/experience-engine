...
test('resolveCommand maps init to the Node installer (no bash)', () => {
  const spec = resolveCommand('init', ['--yes']);
  assert.equal(spec.cmd, process.execPath);
  assert.equal(spec.args[0], path.resolve(__dirname, '..', 'bin', 'init.js'));
