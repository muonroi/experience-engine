 ...
 *
 * Usage:
 *   node tests/manual/detect-experience.js
 *   node tests/manual/detect-experience.js --session <path-to-jsonl>
 *   node tests/manual/detect-experience.js --all   (scan all recent sessions)
 */
'use strict';


// Resolved against the directory this test used to live in, next to the code it tests.
const __srcdir = require('node:path').join(__dirname, '../../.experience/tests');
const fs = require('fs');
const path = require('path');
const os = require('os');

// Load the modules under test
const contextPath = path.join(__srcdir, '..', 'src', 'context.js');
const extractCompactPath = path.join(__srcdir, '..', 'extract-compact.js');
const stopExtractorPath = path.join(__srcdir, '..', 'stop-extractor.js');
