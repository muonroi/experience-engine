 ...
 *
 * Run: node --test tests/tools/exp-stats.test.js
...
describe('computeStats', () => {
  it('computes correct stats for mixed events', () => {
    const events = [
      ...
      { ts: '2026-04-09T00:03:00Z', op: 'extract', experiences: 3, stored: 2, project: 'D:/proj/c.ts' },
      { ts: '2026-04-09T00:04:00Z', op: 'extract', experiences: 1, stored: 0, project: null },
  ...
  it('groups per-project correctly with null -> "(unknown project)"', () => {
    const events = [
      ...
      { ts: '2026-04-09T00:02:00Z', op: 'extract', experiences: 2, stored: 1, project: null }
    ];
    const s = computeStats(events);
...
describe('loadEvents', () => {
  it('reads valid JSONL and skips malformed lines', () => {
    ...
    const lines = [
      ...
      'this is not json',
      JSON.stringify({ ts: '2026-04-09T00:01:00Z', op: 'extract', experiences: 1, stored: 1 }),
  ...
  it('reads rotated .1 file too', () => {
    ...
    fs.writeFileSync(logFile, JSON.stringify({ ts: '2026-04-09T00:00:00Z', op: 'extract', experiences: 1, stored: 1 }) + '\n');
