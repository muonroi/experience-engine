...
- **Language/Framework gate** — `.ts` file → only TypeScript / `scope.lang=all` experiences pass the Qdrant index-level filter; `.cs` file inside a `.csproj` tree → only C# / `dotnet` / `any`. Org-specific framework labels (e.g. shared internal library) are opt-in via `org.frameworkPackages` in `~/.experience/config.json` — the engine itself ships with zero hardcoded framework names.
