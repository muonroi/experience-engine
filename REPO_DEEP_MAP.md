...
| `POST /api/recall` | Active agent self-query in **hybrid semantic-search mode** (`recallMode`): runs a **dense-vector leg + lexical full-text leg** and fuses per tier with **RRF** (`src/fusion.js` `hybridFuse`/`rrfFuse`/`lexicalRank`), so exact-term matches the embedding misses still surface. Casts a wider net (`recallTopK` per collection) and drops the passive-hint noise-control gates (positive lang/project/framework scope filters + min-search-score floor) so cross-repo/-language lessons surface; integrity gates kept (superseded / permanent-noise / irrelevant≥3 / learned exclusions / min-confidence). Lexical leg = Qdrant full-text index on top-level `text_search` payload (written by `evolution.js upsertEntry` via `format.js buildTextSearch`; index ensured at startup in `server.js ensureCollections`; existing points filled by `tools/backfill-text-search.js`); degrades to vector-only if index missing. Spans T0/T1/T2/self-QA, formatted with `[id col]` + 3 feedback verdicts (`followed`/`ignored`/`noise`), records SURFACE (not hit). Wiring: `server.js handleRecall` → `interceptWithMeta(…, {recallMode:true})`; `qdrant.js searchCollectionLexical/ensureTextIndex/setPayloadFields`; `scoring.js rerankByQuality({preserveOrder})` + `format.js formatPoints({skipSearchScoreGate})` |
